...
const GameScreen = ({ navigation }) => {
  ...
  const makeMove = (col) => {
    ...
    if (checkWinner(row, col)) {
      newResultMessage = `${player} e o melhor jogador de quatro em linha!`;
    } else if (checkDraw()) {
      newResultMessage = 'Foi um bom jogo mas acabou em um empate!';
    }
  ...
  const resetGame = () => {
    ...
    if (checkWinner(row, col) || checkDraw()) {
      const result = checkWinner(row, col) ? `${player} e o melhor jogador de quatro em linha!` : 'Foi um bom jogo mas acabou em um empate!';
  ...
  return (
    <View>
      ...
      <Text></Text>
      <Text></Text>
      <Button title="Voltar ao menu principal" onPress={() => navigation.goBack()} color="green" />
      <Text></Text>

      <Button title="Histórico" onPress={() => navigation.navigate('History')} color="orange"/>
      <Modal isVisible={showModal}>
        <View style={styles.modalContent}>
          <Text>{resultMessage}</Text>
          <Button title="Reiniciar" onPress={resetGame} color="green"/>
        </View>
      </Modal>
    </View>
  );
